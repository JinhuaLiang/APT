...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        if (self.enable_fusion) and (self.fusion_type == 'channel_map'):
            self.proj = nn.Conv2d(in_chans * 4,
                                  embed_dim,
        ...
        if (self.enable_fusion) and (self.fusion_type
                                     in ['daf_2d', 'aff_2d', 'iaff_2d']):
            self.mel_conv2d = nn.Conv2d(in_chans,
                                        embed_dim,
    ...
    def forward(self, x, longer_idx=None):
        if (self.enable_fusion) and (self.fusion_type
            ...
            if len(longer_idx) > 0:
                ...
                local_x = local_x.view(B, C, local_x.size(1), local_x.size(2),
                                       local_x.size(3))
                local_x = local_x.permute(
                    (0, 2, 3, 1, 4)).contiguous().flatten(3)
                ...
                global_x[longer_idx] = self.fusion_model(
                    global_x[longer_idx], local_x)
            x = global_x
        else:
            B, C, H, W = x.shape
            assert H == self.img_size[0] and W == self.img_size[1], \
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if self.norm_before_mlp == 'ln':
            ...
            self.norm2 = lambda x: nn.BatchNorm1d(dim)(x.transpose(1, 2)
                                                       ).transpose(1, 2)
        else:
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        SF = self.spec_size // (2**(
            len(self.depths) - 1)) // self.patch_stride[0] // self.freq_ratio
        self.tscam_conv = nn.Conv2d(in_channels=self.num_features,
    ...
    def forward_features(self, x, longer_idx=None):
        ...
        fpx = interpolate(
            torch.sigmoid(x).permute(0, 2, 1).contiguous(),
            8 * self.patch_stride[1])
    ...
    def crop_wav(self, x, crop_size, spe_pos=None):
        ...
        tx = torch.zeros(x.shape[0], x.shape[1], crop_size,
    ...
    def reshape_wav2img(self, x):
        ...
        if T < target_T:
            x = nn.functional.interpolate(x, (target_T, x.shape[3]),
                                          mode="bicubic",
                                          align_corners=True)
        if F < target_F:
            x = nn.functional.interpolate(x, (x.shape[2], target_F),
                                          mode="bicubic",
                                          align_corners=True)
    ...
    def forward(self,
                ...
                mixup_lambda=None,
                infer_mode=False,
                device=None):  # out_feat_keys: List[str] = None):

        if self.enable_fusion and x["longer"].sum() == 0:
            # if no audio is longer than 10s, then randomly select one audio to be longer
            if self.training:
                x["longer"][torch.randint(0, x["longer"].shape[0],
                                          (1, ))] = True
            else:
        ...
        if not self.enable_fusion:
            ...
            if self.fusion_type in ['daf_1d', 'aff_1d', 'iaff_1d']:
                ...
                if len(longer_list_idx) > 0:
                    # local processing
                    fusion_x_local = x[longer_list_idx,
                                       1:, :, :].clone().contiguous()
                    FB, FC, FT, FF = fusion_x_local.size()
                    fusion_x_local = fusion_x_local.view(FB * FC, FT, FF)
                    fusion_x_local = torch.permute(fusion_x_local,
                                                   (0, 2, 1)).contiguous()
                    fusion_x_local = self.mel_conv1d(fusion_x_local)
                    fusion_x_local = fusion_x_local.view(
                        FB, FC, FF, fusion_x_local.size(-1))
                    fusion_x_local = torch.permute(
                        fusion_x_local, (0, 2, 1, 3)).contiguous().flatten(2)
                    if fusion_x_local.size(-1) < FT:
                        fusion_x_local = torch.cat([
                            fusion_x_local,
                            torch.zeros((FB, FF, FT - fusion_x_local.size(-1)),
                                        device=device)
                        ],
                                                   dim=-1)
                    else:
