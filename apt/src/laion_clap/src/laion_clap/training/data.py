...
_AUDIOSET_MAP_PATH = os.path.join(
    Path(__file__).parent, "audioset_textmap.npy")
...
class ToyDataset(Dataset):
    ...
    def prompt_text(self, target):
        ...
        event_text = "The sounds of " + ", ".join(
            events[:-1]) + " and " + events[-1]
    ...
    def __getitem__(self, index):
        ...
        hdf5_path = (self.fp["hdf5_path"][s_index].decode().replace(
            "../workspace",
        ...
        with h5py.File(hdf5_path, "r") as f:
            waveform = int16_to_float32(
                f["waveform"][r_idx])[:self.audio_cfg["clip_samples"]]
        ...
        mel_spec = get_mel(torch.from_numpy(waveform),
                           self.audio_cfg)[None, :, :]
        mel_spec = torch.cat(
            [mel_spec,
             mel_spec.clone(),
...
def get_mel(audio_data, audio_cfg):
    ...
    mel_tf = torchaudio.transforms.MelSpectrogram(
        ...
        f_max=audio_cfg['fmax']).to(audio_data.device)
    ...
    with grad_fn():
        if len(audio_data) > max_len:
            if data_truncating == "rand_trunc":
                ...
                chunk_frames = max_len // audio_cfg[
                ...
                if chunk_frames == total_frames:
                    ...
                    # select mel
                    mel_chunk_front = mel[idx_front:idx_front +
                                          chunk_frames, :]
                    ...
                    mel_fusion = torch.stack([
                        mel_shrink, mel_chunk_front, mel_chunk_middle,
                        mel_chunk_back
                    ],
                                             dim=0)
                    sample["mel_fusion"] = mel_fusion
...
def preprocess_single(
    ...
    sample = get_audio_features(sample, audio_data, max_len, data_truncating,
    ...
    if isinstance(texts, list) and isinstance(texts[0],
                                              str) and len(texts) > 1:
    ...
    if class_index_dict is not None:
        ...
        class_labels[np.in1d(list(class_index_dict.keys()),
                             json_dict_raw["tag"])] = 1
...
def collate_fn_with_preprocess(
    ...
    class_index_dict = copy.deepcopy(
        args.class_index_dict)  # To avoid deadlock in multiprocessing
    ...
    for sample in batch:
        data_preprocessed.append(
            preprocess_single(sample, audio_ext, text_ext, max_len, audio_cfg,
                              tmodel, class_index_dict, data_filling,
                              data_truncating, text_augment_selection))

    batch_dict = {}
    for k in data_preprocessed[0].keys():
        if isinstance(data_preprocessed[0][k],
                      dict):  # dealwith bert tokenizer output
            batch_dict[k] = {}
            ...
            batch_dict[k] = torch.stack(
                [sample[k] for sample in data_preprocessed])
    ...
    if not num_samples:
        if is_train:
            ...
            num_samples = (
                ...
                or 0)  # eval will just exhaust the iterator if not specified
...
def wds_batch_list2dict(
    ...
    """
    assert len(keys) == len(
        batch), "batch must have same number of keys as keys argument"
...
def get_data(args, model_cfg):
    ...
    if args.train_data:
        data["train"] = get_dataset_fn(args.dataset_type)(args,
                                                          model_cfg,
                                                          is_train=True)

    if args.val_data:
        data["val"] = get_dataset_fn(args.dataset_type)(args,
                                                        model_cfg,
                                                        is_train=False)
