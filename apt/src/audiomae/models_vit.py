...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def random_masking_2d(self, x, mask_t_prob, mask_f_prob):
        ...
        index = ids_keep.unsqueeze(-1).unsqueeze(-1).repeat(
            1, 1, len_keep_T, D)
        x_masked = torch.gather(x, dim=1, index=index)
    ...
    def forward(self, x, v=None, mask_t_prob=0.0, mask_f_prob=0.0):
        ...
        class_embedding = self.head(x["class_embedding"])
        return {
            "patch_embedding": x["patch_embedding"],
            "class_embedding": class_embedding,
            "audio_embedding": x['audio_embedding'],
        }
